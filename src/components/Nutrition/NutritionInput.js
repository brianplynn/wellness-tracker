					    ...
					    <input id="edamame" 
					   	...
					   	{ !isSearching ? 
					   		...
					   		( isPending ?
					   			...
					   			( error ? 
					   			<div className="error bg-white pa2 w-100 tl">No results found.</div> :
					   			<div className="results-container bg-white w-100">
					   				{ searchResults.map( (result, i) => (
					   					<div key={i} 
					   						 onClick={selectEdamame.bind(null, searchResults[i].food)}
					   						 className="result pa2 hover-bg-light-gray pointer bg-white w-100">
					   						<span className="dark-green">{result.food.brand ? result.food.brand + " " : ""}</span>
					   						{result.food.label + " "}
					   						<span className="purple">({Math.round(result.food.nutrients.ENERC_KCAL)} kcal)</span>
					   					</div>
					   					)) 	
					   				}
					   			</div>
